...
/**
 * {@code StateDefinition} represents a state in MXML 2009 or later.
 * States are per-class, so these objects are owned
 * by the {@code ClassDefinitionNodes} that define classes in MXML.
 */
public class StateDefinition extends StateDefinitionBase implements IStateDefinition
{
     ...
     * Constructor.
     */
    public StateDefinition(IMXMLStateNode node, IASScope containingScope, 
    ...
    /**
     * A map mapping group names to {@code IStateGroup} objects.
     * This is effectively the set of groups that include this state,
     * but we use a map because we're interested in this set
     * both as a set of group names (the String keys)
     * and a set of group objects (the {@code IStateGroup} values).
     */
    ...
    public String[] getStateGroups()
    {
        return groupMap != null ? 
               groupMap.keySet().toArray(new String[0]) : 
               new String[0];
    }
