...
import org.apache.royale.swf.builders.FontBuilder;
...
import org.apache.royale.utils.LRUCache;
...
public abstract class CachedFontManager extends FontManager
{
    ...
    public DefineFontTag createDefineFont(int tagCode, FontDescription font)
    {
        if (font.compactFontFormat)
        {
            throw new FontManager.EmbedAsCffNotSupportedException();
        }
        FontFace fontFace;
        TagType tagType = TagType.getTagType(tagCode);
        boolean useTwips = TagType.DefineFont3.equals(tagType);
        ...
        return (DefineFontTag)builder.build();
    }

    public void loadDefineFont(DefineFontTag tag, Object location)
    {
        if (tag instanceof DefineFont3Tag)
        {
            loadDefineFont3((DefineFont3Tag)tag, location);
        }
    }

    protected void loadDefineFont3(DefineFont3Tag tag, Object location)
    {
        String family = null;
        ...
        else
        {
            ...
            if (face instanceof CachedFontFace)
            {
                ...
                int[] codepoints = tag.getCodeTable();
                if (codepoints != null)
                {
