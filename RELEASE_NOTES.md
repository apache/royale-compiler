...
- compiler: Fixed missing symbols specified with `-include-resource-bundles` compiler option in generated JS.
- compiler: Fixed missing "Generated byte code contains an operand stack underflow" error when warnings are reported without other errors.
- compiler: Fixed null exception when emitting embedded plain text to JS.
- compiler: Fixed operand stack underflow error when using data binding in MXML to assign to `<fx:Object>` property.
